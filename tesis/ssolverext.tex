...
Hemos incluido ambos problemas en el \emph{benchmark} para la evaluación de la herramienta. Ambos son problemas difíciles que requieren exponencialmente más tiempo de análisis conforme crece la cantidad de \texttt{Node}s en el \emph{scope}. Sin embargo, el segundo problema (el de demostrar \texttt{Completeness}) exhibe una curva de dificultad aún mucho más pronunciada que el primero.
...
\begin{itemize}
	\item si partimos de la base de que tenemos \hard disponible 
	\item Los speedups conseguidos para cada problema a medida que crece el scope son crecientes en todos los casos.
	\item Esto impacta en mayor eficiencia con el scope
	\item Baja eficiencia en general
\end{itemize}

En cuanto a los resultados obtenidos se observa que en general la herramienta
consiguió disminuir el tiempo necesario para resolver un problema, aunque las
proporciones en que logró hacerlo son sumamente diversas. Las mismas varían
desde prácticamente nada
