...
--CREATE ALIAS P8 AS ' ResultSet p8(Connection conn, int sorder) throws SQLException { String query = "(SELECT * FROM TIMES_PER_FILTER_AND_LIFTER WHERE \"cnf\" LIKE ''%p8%'' LIMIT "+new Integer(sorder * 28 + 0).toString()+", 10) UNION ALL (SELECT * FROM TIMES_PER_FILTER_AND_LIFTER WHERE \"cnf\" LIKE ''%p8%'' LIMIT "+new Integer(sorder * 28 + 11).toString()+", 17) UNION ALL (SELECT * FROM TIMES_PER_FILTER_AND_LIFTER WHERE \"cnf\" LIKE ''%p8%'' LIMIT "+new Integer(sorder * 28 + 10).toString()+", 1)"; return conn.createStatement().executeQuery(query); } ';