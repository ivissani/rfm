...
In the introduction we mentioned the general objectives of the project by identifying some new aspects of the discipline of software artifacts design and development. They can be synthesized by recalling on:
\begin{inparaenum}[\itshape a\upshape)]
\item the heterogeneity derived from the fact that nowadays the languages for software design and development are conceived as a collection of languages designed to reflect particular behaviors,
\item the lack of formal foundations for service oriented software, this new conception of a software artifacts as a collection of services that are dynamically discovered and binded at runtime are not well supported by the traditional formal foundations of heterogeneity, and
...
Explicar en esta sección cómo nuestra experiencia en:

- formalizaciones de heterogeneidad y especificaciones estructuradas
- logicas no-monótonas: entailment systems for non-monotonic logics
- formalización de herramientas semánticas: satisfiability calculus
- herramientas: demostración de teoremas y bounded model checking de cómputo intensivo
- formalización para SOC: dynamic reconfiguration

se integra completa en la hipótesis.

%%%


In this section we will present the general hypothesis of the project and some aspects of the methodology we will adopt to develop the objectives we presented in previous sections. 

In the previous section we described a new conception of software development based on the idea that a software artifact is a collection of services that are discovered, binded and coordinated at runtime instead of the traditional point of view that assumes there is a closed world in which every component is present and completely described at design time. Traditional approaches present two main drawbacks that require formal foundations to be provided:
\begin{inparaenum}[\itshape a\upshape)]
...
\end{inparaenum}
Our working hypothesis regarding service oriented software construction is based on this lack of formal foundations for this new paradigm of software development. In this sense we will focus mainly on the theoretical development of a framework capable of characterizing this class of software systems. Only at the end of the project, and once the theoretical foundations get consolidated, we will evaluate the possibility of constructing tools based on them.\\

One of the main concerns of the project is related to the fact formal methods must be accompanied by tools supporting them, not only as a proof of concept but as a genuine possibility of applying them in order to obtain better quality software. Nowadays there exists a lot of tools implementing a diversity of formal methods but usually they serve more as a justification of the potential application of formal methods than as real tools available for industrial strength software construction. Regarding this aspect of the project we will follow a more practical approach focussing on the implementation of both formal tools and new improved interfaces that resort to new technology in HCI (human computer interface).

As the provision of tool support for SOC depend on the formalization of formal foundations supporting it, our work will concentrate on the continuation of the already open research lines in theorem proving and bounded model checking of {\sf Alloy} and fork algebras specifications. As we will describe in the next section, the direction of our research is the generalization of the tool Dynamite; in this sense the project will enrich our experience in the development of tools for formal methods.\\

In the next section we will go deeper in the particular hypothesis that will rule each of the aspects of the project. In addition, we will present a detailed, as far as it can be done, explanation of some of the tasks we plan to develop during the project.
...
As we explained before there are three main areas of interests in which the contributions of the group will be focalized. Throughout this section we will consider heterogeneity as a constituent aspect of the general problem we are adopting as a guideline for the project, which is the exploration of service oriented software construction in order to provide formal foundations for this new paradigm. In order to make this presentation more friendly we will try to separate the hypothesis from the methodology, being the hypothesis a succinct characterization of the problems we want to solve, and the methods an introduction to what we believe is the correct approach to solve them.


\subsection*{Service Oriented Computing and heterogeneous languages for software specification}
We already mentioned how global ubiquitous computing has moved the focus of software construction from a static closed world view to a more dynamic and constantly changing view in which a software must be understood as a collection of services that are discovered and binded at runtime by means of a service level agreements negotiation. This frameworks are usually grouped under the name of Service-Oriented Computing (SOC). We also mentioned how this new conception brings a complete new level of heterogeneity and how the state of the art approaches for heterogeneous software specification, those coming from the field of algebraic specification and category theory, fail to characterize. 

The first step in this direction will be the evaluation of the state of the art in the search of the limits the traditional approach to heterogeneity has when it is applied to this new conception of software artifact. We believe that the key concept here is the notion of interoperability of software pieces instead of the traditional communication over a common language usually identified with communication channels. Instead, interoperability is more related to the concept of port coming from the field of software architecture. There, the communication channel has its own behavior, similar to the role of a communication protocol which rules the negotiation of the terms under which the software components will be binded in order to provide the service. 
...
This should generalize, in the case where the languages are institutions, the concept of \emph{channel} between the fork algebraic representations of logical specifications that I defined in \cite{lopezpombo:phdthesis} based on \cite{fiadeiro:amast96}.  In this new framework, the connector-based notion of channel should overcome problems that arise when working with Grothendieck institutions due to the fact that relations are established directly between logical descriptions by means of morphisms, which necessarily include the use of a morphism/representation map between the different institutions. 
...
With regard of this aspect of the project contributions are expected to be more theoretical and related to the formal foundations of this new paradigm of software construction. We believe that providing tool support for this kind of framework requires a maturation time so we only plan to explore this practical aspects at the end (last year) of the project depending of the advances we were able to make.
...
The actual stage in the development of this tool is the use of the bounded model checker to propose relational terms to be used as witnesses in the proof of properties existentially quantified.  

%%%

Regarding the development of tool support we have separated the work under two separate working hypothesis. The first one is the need for a re-engineering of Dynamite to turn it into a general interface for the validation and verification of specifications. To accomplish this several tasks must be developed. Todays version of Dynamite is an adapted version of PVS that has the following drawbacks: 
\begin{inparaenum}[\itshape a\upshape)]
\item the theorem prover runs as a process of Emacs, its interface, with no real independence,
\item the relation between the theorem prover and the bounded model checker is asymmetric because {\sf Alloy} is used by PVS as a validation tool, and
\item there is no easy to use and intuitive way to work with a specification.
\end{inparaenum}

Our first step will be to decouple PVS from Emacs enabling the theorem prover to run as an independent process interacting through a daemon listening in a users defined port. This also enables the possibility of PVS to be running on a separate machine that provides theorem proving as a service. The same thing can be done with {\sf Alloy} thus, separating the interface of the tool from the services of validation and verification of specifications. At this point PVS no longer needs to implement strategies that invoke {\sf Alloy} as a slave procedure. Now, this strategies can be moved to more appropriate level of abstraction in which they are the result of a coordination of different formal methods with different purposes in the process of software construction.
...
\begin{itemize}
\item {\bf improved information presentation}: we will implement modern techniques of information presentation to enable a more intuitive manipulation of proofs and counterexamples in a completely graphical environment. All theorem provers are based in text mode interfaces and we believe this is one of the reasons why this kind of tools are not included in industrial software development processes. This task requires a complete reengineering of the Dynamite.
...
Once of the problems we will work on is the adaptation of the learning techniques in order to implement them as a part of {\sf ParAlloy}.
