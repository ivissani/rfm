...
case class SolveThisMessage(i: Iteration)
...
class IterationSolverActor extends Actor {
  ...
  object InternalActor extends Actor {
        ...
        if (s.simplify()) {
          ...
          if (its <= 1) {
            ...
            if (res == 'I') {
              ...
              val toLift = definition.lifter.variablesToLift()(s).sort((a: Int, b: Int) ⇒ a.abs < b.abs)
    ...
    def act() {
      loop {
        react {
          case SolveThisMessage(i) => iterate(i.definition, i.expId, i.pId, i.level, i.its, i.path, i.withLearnts, i.assumedByParent, i.forMeToAssume)
        }
      }
    }
  }
  ...
  def act() {
    loop {
      react {
        case SolveThisMessage(i) => InternalActor ! SolveThisMessage(i)
