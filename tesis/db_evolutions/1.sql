...
--CREATE ALIAS RESULTS_FOR_TABLE AS ' ResultSet results_for_table(Connection conn, String cnf, int sorder) throws SQLException { String query = "(SELECT * FROM TIMES_PER_FILTER_AND_LIFTER WHERE \"cnf\" LIKE ''%"+cnf+"%'' LIMIT "+new Integer(sorder * 28 + 0).toString()+", 10) UNION ALL (SELECT * FROM TIMES_PER_FILTER_AND_LIFTER WHERE \"cnf\" LIKE ''%"+cnf+"%'' LIMIT "+new Integer(sorder * 28 + 11).toString()+", 17) UNION ALL (SELECT * FROM TIMES_PER_FILTER_AND_LIFTER WHERE \"cnf\" LIKE ''%"+cnf+"%'' LIMIT "+new Integer(sorder * 28 + 10).toString()+", 1)"; return conn.createStatement().executeQuery(query); } ';

--CREATE ALIAS BORRAR_EXPERIMENTO AS 'void borrar_experimento(Connection conn, int id) throws SQLException { conn.createStatement().execute("DELETE FROM ASSUMED_LITERALS WHERE \"iteration_id\" IN (SELECT \"id\" FROM ITERATIONS WHERE \"experiment_id\"=" + new Integer(id).toString() + ")"); conn.createStatement().execute("DELETE FROM ITERATIONS WHERE \"experiment_id\"=" + new Integer(id).toString()); conn.createStatement().execute("DELETE FROM EXPERIMENTS WHERE \"id\"=" + new Integer(id).toString()); }';