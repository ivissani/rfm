...
class H2ExperimentLogger extends ExperimentLogger {
  ...
  def initialize(ses : Session) {
      ...
      val tables = makeTableMap(ses)
      if (!tables.contains(Experiments.tableName)) { Experiments.ddl create ses}
      if (!tables.contains(Iterations.tableName)) { Iterations.ddl create ses}
      if (!tables.contains(AssumedLiterals.tableName)) { AssumedLiterals.ddl create ses}
  }
